...
public class InvoicePaymentDetailValidator extends AbstractValidator<InvoicePaymentDetail> {
    ...
    public String validateMoney(String fieldName, Money money) {
        if (money == null) {
            return DomainExceptionCauses.requiredField(fieldName);
        }
        if (!IntegerHandler.isGreaterThanZero(money.amount())) {
            return DomainExceptionCauses.illegalArgument(fieldName);
        }
        return EnumHandler.validateEnum(moneyCurrencyEnumExists, fieldName + " Currency", money.currency());
    }
